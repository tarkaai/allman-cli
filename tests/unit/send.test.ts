...
const convRecord: ConversationRecord = {
  ...
  participants: [
    ...
    { profileId: CONTACT_PROFILE_ID, slug: "example-user-1", urn: `urn:li:fsd_profile:${CONTACT_PROFILE_ID}`, name: "James Foo" },
  ],
  unreadCount: 0,
...
describe("send command — pre-send sync abort", () => {
  ...
  beforeEach(async () => {
    ...
    // Write conversation + alias so "example-user-1" resolves to it
    const { conversations } = store.forAccount(MY_PROFILE_ID);
    await conversations.upsert(CONV_BARE_ID, convRecord);
    await conversations.createAlias("example-user-1", CONV_BARE_ID);
  });
