...
export async function conversationsCommand(options: ConversationsOptions): Promise<void> {
  ...
  if (bareIds.length === 0) {
    info(`No conversations found. Run \`allman sync\` to pull history.`);
    return;
  }
  ...
  const records = (
    ...
    .sort((a, b) => {
      const at =
        a.record.syncState?.newestMessageAt ?? new Date(a.record.lastActivityAt ?? 0).getTime();
      const bt =
        b.record.syncState?.newestMessageAt ?? new Date(b.record.lastActivityAt ?? 0).getTime();
      return bt - at;
    })
    .slice(0, limit);
  ...
  const lines = records.map((r) => {
    const rec = r.record;
    const timeMs =
      rec.syncState?.newestMessageAt ??
      (rec.lastActivityAt ? new Date(rec.lastActivityAt).getTime() : 0);
